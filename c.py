...
@app.route('/attendance/<class_code>', methods=['GET', 'POST'])
def attendance(class_code):
    if request.method == 'POST':
        action = request.form['action']
        if action == 'attendance':
            class_code = request.form['class_code']
            name = take_attendance(class_code)
            return render_template('attendance.html', class_code=class_code, name=name)
...
def capture():
    ...
    if action == 'register':
        ...
        class_code = request.form['class_code']
        name = take_attendance(class_code)
        return redirect(url_for('attendance', class_code=class_code, name=name))
...
def take_attendance(class_code):
    ...
    wb = xl_copy(rb)
    sheet = wb.get_sheet(switch_case(class_code))
    if sheet is None:
    ...
    while True:
        ...
        cv2.imshow('Video', frame)
        k = cv2.waitKey(1)
        if k % 256 == 27 or recognized_name:  # ESC to exit or recognized a face
            print("Data saved")
            break
    video_capture.release()
    cv2.destroyAllWindows()
